#' Pairwise Total Variation Distance Lower Bound Matrix in Multi-Class Settings
#'
...
#' @param ... additionnal parameters to be passed to the dWit function.
#'
...
#'  tvhat.iris <- dWit(labels = y[-ind.train], ordering.array = ar)
#'  tvhat.iris
#'
#' @export
dWitMatrix <- function(labels,
                       ordering.array,
                       alpha            = 0.05,
  ...
  if (!is.numeric(labels) || any(!is.finite(labels)) || is.matrix(labels) || any(!(labels %in% c(0:length(unique(labels)-1))))) {
    stop("Invalid values or type for labels, please see ?dWitMatrix.")
  }
  ...
  if (!is.character(computation.type) || (length(computation.type) != 1) || !(computation.type %in% c("non-optimized"))) {
    stop("Invalid values for computation.type, please see ?dWitMatrix.")
  }

  # alpha check
  if (!is.numeric(alpha) || (length(alpha) != 1) || (alpha > 1) || (alpha <= 0)) {
    stop("Invalid type I error level alpha, please see ?dWitMatrix.")
  }
  ...
  if (is.null(dim(ordering.array))) {
    ...
    for (i in 0:(nclass-2)) {
      ...
      for (j in (i+1):(nclass-1)) {
        ...
        # lower bound
        tvhat.mat[i+1, j+1] <- dWit(t         = as.numeric(labels == j)[ind.pairs],
    ...
    for (i in 0:(nclass-2)) {
      ...
      for (j in (i+1):(nclass-1)) {
        ...
        if (sum(labels == i) <= 1 | sum(labels == j) <= 1) {
          ...
          warning("at least one class has less than one representative, lower bound set to 0 by default, see ?dWitMatrix.")
