...
def save_predictions(loader, is_gpu):
    """Calculate accuracy.
    Args:
        loader (torch.utils.data.DataLoader): training / test set loader
        is_gpu (bool): whether to run on GPU
...
for epoch in range(start_epoch, opt.epochs + start_epoch):
    ...
    if epoch % 50 == 0:
        ...
        predictions, labels = save_predictions(tvloader, opt.is_gpu)
        preds = {'predictions': predictions.cpu().detach().numpy(),'labels': labels.cpu().detach().numpy()} 
        with open('./predictions_cifar10.pkl', 'wb') as f: 
            pickle.dump(preds, f)
